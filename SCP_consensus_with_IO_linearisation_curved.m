...
dR_q = [S_omega*R_q(1:2,:); 0 0];
dR_q_func = Function('dR_q_func', {q, omega}, {dR_q});
...
for l=2:T
    ...
    ddq_l = R_q_func(q_l)*dv_l(:, l-1) + dR_q_func(q_l, dq_l(3))*v_l(:, l-1);
    p_l(:, l) = q_l;
    dp_l(:, l) = full(dq_l);
    ddp_l(:, l) = full(ddq_l);
    Delta_history(:, :, l) = full(R_theta_func(q_l(3))*Delta);
    dDelta_history(:, :, l) = full(S_omega_func(dq_l(3))*R_theta_func(q_l(3))*Delta);
    ddDelta_history(:, :, l) = full(S_omega_func(ddq_l(3))*R_theta_func(q_l(3))*Delta...
...
save('Curved_Trajectory_Values.mat', 'formation_error', 'q_i_library', 'p_obs', 'obstacle_radius',...
